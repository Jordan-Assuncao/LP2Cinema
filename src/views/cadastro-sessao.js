...
const baseURL4 = `${BASE_URL4}/sessaotipoexibicoes`;
    ...
    if (idParam == null) {
      try {
        ...
        if (tipoExibicao.length > 0) {
          ...
          mensagemSucesso(`Sessâo cadastrada com sucesso!`);
        } else {
          mensagemSucesso(`Sessão cadastrada sem tipos de exibição!`);
        }

        navigate(`/listagem-sessoes`);
      } catch (error) {
        mensagemErro(error?.response?.data || 'Erro ao salvar a Sessão');
      }
        ...
        <div className='row'>
          <div className='col-lg-12'>
            <div className='bs-component'>
              ...
              <FormGroup label='Preço da Sessão (R$): *' htmlFor='inputIdPreco'>
                <select
                  type='text'
                  id='inputIdPreco'
                  value={idPreco}
